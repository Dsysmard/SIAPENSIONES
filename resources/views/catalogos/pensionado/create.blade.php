...
<h1>REGISTRAR UN NUEVO PENSIONADO</h1><hr/>
</div>
	<div class="row jumbotron " style="padding-top: 20px;border-top-width: 20px;margin-top: 30px;">
		<div class="col-md-10 col-md-offset-1"style="margin: 2% 50px 75px 100px;">
            {!! Form::open(['route' => 'pensionado.store', 'method' => 'post', 'novalidate']) !!}
      ...
      <div class="form-group col-md-6 {{ $errors->has('Fecha_final_lab') ? 'has-error' : '' }}">
          ...
          <span class="text-danger">{{ $errors->first('Fecha_final_lab') }}</span>
        </div>
      ...
      <div class="form-group col-md-6">
          ...
          <select class="form-control {{ $errors->has('modpension') ? 'has-error' : '' }}" id="modo_pension" name="modo_pension" >
              @foreach($pension as $modpension)
              <option>{{$modpension->descripcion}}</option>
              <span class="text-danger">{{ $errors->first('modpension') }}</span>
              @endforeach
          </select>
      </div>
